...
def detect_ArUco_details(image):
    ...
    tupids=tuple(ids)
    tupcorners=tuple(corners)


    for i,corner in zip(tupids,tupcorners):
        ...
        tupcenter=tuple(center)
        m = (corner[0][0][1]-int(centerY))/(corner[0][0][0]-int(centerX))
        angle= np.arctan2(m)
        if (angle*57.2958+90)<=180:
            orangle= int((angle*57.2958)+90)
        else:
            orangle= int(-((angle*57.2958)+90))
        
        ArUco_details_dict[i]=[tupcenter]
        
    for i,corner in zip(ids,corners):
        ArUco_corners[i] = corner
